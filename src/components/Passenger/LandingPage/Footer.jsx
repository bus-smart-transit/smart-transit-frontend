import { Link } from 'react-router-dom';
import { Bus } from 'lucide-react';

export default function Footer() {
  return (
    <footer className="border-t border-slate-800 bg-slate-950 px-4 py-10 sm:px-6 lg:px-10">
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col items-center justify-between gap-6 sm:flex-row">
          {/* Brand */}
          <Link to="/" className="inline-flex items-center gap-2.5" aria-label="SmartTransit home">
            <span className="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-blue-600 text-white">
              <Bus className="h-4 w-4" aria-hidden="true" />
            </span>
            <span className="text-sm font-bold text-slate-300">
              Smart<span className="text-sky-400">Transit</span>
            </span>
          </Link>

          {/* Copyright */}
          <p className="order-last text-xs text-slate-600 sm:order-none">
            © {new Date().getFullYear()} SmartTransit. All rights reserved.
          </p>

          {/* Quick links */}
          <nav className="flex items-center gap-5 text-xs text-slate-500" aria-label="Footer navigation">
            <Link to="/passenger/login" className="transition hover:text-slate-300">
              Passenger Login
            </Link>
            <Link to="/passenger/signup" className="transition hover:text-slate-300">
              Sign Up
            </Link>
            <Link to="/employee/login" className="transition hover:text-slate-300">
              Staff Login
            </Link>
          </nav>
        </div>
      </div>
    </footer>
  );
}
