...
import { Bus, Menu, X, LayoutGrid, Ticket, History, MapPin, Gift, User, LogOut } from 'lucide-react';
...
const ACCOUNT_LINKS = [
  { label: 'Dashboard', to: '/passenger/dashboard?tab=dashboard', icon: LayoutGrid },
  { label: 'My Tickets', to: '/passenger/dashboard?tab=tickets', icon: Ticket },
  { label: 'Trip History', to: '/passenger/dashboard?tab=transactions', icon: History },
  { label: 'Track Bus', to: '/passenger/dashboard?tab=map', icon: MapPin },
  { label: 'Rewards', to: '/passenger/dashboard?tab=rewards', icon: Gift },
  { label: 'Profile', to: '/passenger/dashboard?tab=profile', icon: User },
];
...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-navy-950">
      ...
      {open && (
        <nav
          id="mobile-nav"
          className="border-t border-slate-800 bg-navy-950 px-4 pb-5 pt-3 md:hidden"
              ...
              {isAuthenticated ? (
                <>
                  ...
                  <button
                    ...
                    className="mt-1 flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm font-medium text-red-400 transition hover:bg-slate-800"
                  >
                    <LogOut size={18} />
                    Logout
                  </button>
